...
Before reasoning from scratch, check for precedent in this project:
`node scripts/check-precedent.js "<keyword>"` (skip silently if the script
isn't present). If this project already rejected similar scope, apply that
decision directly and say so in one line ("precedent: already rejected X for
Y, doing Y again") instead of re-deriving it.
